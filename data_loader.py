...
def load_data():
    if st.session_state.get('data_source') == 'uploaded' and 'uploaded_df_clean' in st.session_state:
        return st.session_state['uploaded_df_clean'].copy()
...
def generate_sample_data():
    np.random.seed(42)
    n = 10000
    countries = ['United Kingdom', 'Germany', 'France', 'Netherlands', 'Spain', 'Belgium', 'Switzerland', 'Portugal', 'Australia', 'Norway']
    products = [
    ...
    df = pd.DataFrame({
        ...
        'CustomerID': np.random.choice([f'{i}' for i in range(12000, 12500)] + [None]*200, n),
        'Country': np.random.choice(countries, n, p=[0.85, 0.04, 0.03, 0.02, 0.02, 0.01, 0.01, 0.01, 0.005, 0.005])
    })
